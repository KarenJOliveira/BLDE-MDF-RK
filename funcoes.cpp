...
vector<int> retornaCaminho(int startNode, int endNode, double **cost, int maxNodes, vector<int> list, double &rest) {
    vector<int> path;
...
void calculaFuncao(double *ind, int d, int nivel, double *leader, double *follower, int funcao, double &fitness, double &restriction, double **cost, vector<int> tollEdges, int maxNodes, tuple<int, int, double> commodity) {
    ...
    // Problema de tarifação
        ...
        if (nivel == 1) {
            ...
            vector<int> path;
            vector<int> list = ordenaSolucao(follower, 100, source);
            path = retornaCaminho(source, target, cost, maxNodes, list, rest);
            ...
            vector<int> path;
            vector<int> list = ordenaSolucao(ind, d, source);
            path = retornaCaminho(source, target, cost, maxNodes, list, rest);
